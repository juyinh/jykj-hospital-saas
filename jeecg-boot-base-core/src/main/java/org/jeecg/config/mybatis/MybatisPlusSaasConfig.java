...
public class MybatisPlusSaasConfig {
    ...
    static {
        //1.需要租户隔离的表请在此配置,查询的时候会默认添加租户tenantId
        if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
            ...
            TENANT_TABLE.add("his_clinic");
            TENANT_TABLE.add("his_clinic_appoint");
            TENANT_TABLE.add("his_clinic_room");
            TENANT_TABLE.add("his_doctor");
            TENANT_TABLE.add("his_doctor_work");
            TENANT_TABLE.add("his_office");
            TENANT_TABLE.add("his_patient");
            TENANT_TABLE.add("his_patient_case");
        }
    }
...
//    }
    ...
    /*
     *@Description: 设置admin账户不需要租户隔离
     *@Param: []
     *@Return: java.lang.Boolean
     *@author: xiaopeng.wu
     *@DateTime: 15:04 2023/11/30
    **/
    public static Boolean getOpenSystemTenantControl(){
        LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        if (OPEN_SYSTEM_TENANT_CONTROL && !"admin".equals(loginUser.getUsername())) {
            return true;
        }
        return false;
    }
