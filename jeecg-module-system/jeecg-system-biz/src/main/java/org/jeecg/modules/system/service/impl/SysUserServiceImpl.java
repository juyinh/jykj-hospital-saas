...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public void addUserWithRole(SysUser user, String roles) {
		...
		//添加系统默认部门
		sysUserDepartService.save(new SysUserDepart(user.getId(), "1"));
	}
	...
	public Set<String> getUserPermissionsSet(String username) {
		...
		if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
			...
			List<SysPermission> testRoleList = sysPermissionMapper.queryPermissionByTenantRoleId();
			permissionList.addAll(testRoleList);
		}
		//================= end 开启租户的时候 如果没有租户角色，默认加入租户角色================
...
//		}
	...
	public void updateStatus(String id, Integer status) {
		userMapper.update(new SysUser().setStatus(status),
