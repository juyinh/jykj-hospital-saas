...
import org.jeecg.common.util.sms.TYSmsUtil;
import org.jeecg.config.mybatis.MybatisPlusSaasConfig;
import org.jeecg.modules.base.service.BaseCommonService;
import org.jeecg.modules.system.dto.TenantAuditDTO;
import org.jeecg.modules.system.entity.*;
import org.jeecg.modules.system.service.*;
import org.jeecg.modules.system.vo.SysTenantVO;
...
public class SysTenantController {
    ...
    @Value("${jy.scan.redirectUrl}")
    private String redirectUrl;

    @ApiOperation(value = "租户信息-分页列表")
	...
	public Result<IPage<SysTenant>> queryPageList(SysTenant sysTenant,@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
        ...
        //判断用户注册进来是否添加租户信息,注册进来未添加用户直接返回
        if (StrUtil.isBlank(sysUser.getRelTenantIds())){
        ...
        if (!"admin".equals(sysUser.getUsername())) {
            sysTenant.setId(oConvertUtils.getInt(TenantContext.getTenant(), 0));
        }
        //---author:zhangyafei---date:20210916-----for: 租户管理添加日期范围查询---
    ...
    public Result<SysTenantVO> queryTenantDetail() throws IOException, WriterException {
        Result<SysTenantVO> result = new Result<SysTenantVO>();
        //获取登录用户信息
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        SysUserTenant sysUserTenant = relationService.getOne(new QueryWrapper<SysUserTenant>().
                lambda().eq(SysUserTenant::getUserId, sysUser.getId()));
        if (sysUserTenant == null) {
            result.success("医院信息不存在，请填写商家信息！");
            return result;
        }
        ...
        if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL && !"admin".equals(sysUser.getUsername())){
            ...
            if (!loginSessionTenant.equals(0)) {
                if(loginSessionTenant!=null && !loginSessionTenant.equals(Integer.valueOf(sysUserTenant.getTenantId()))){
                    result.error500("无权限访问他人租户！");
                    return result;
                }
            }
        }
        SysTenant sysTenant = sysTenantService.getById(sysUserTenant.getTenantId());
        if(sysTenant==null) {
            result.error500("未找到对应实体");
        }else {
            SysTenantVO sysTenantVO = BeanUtil.toBean(sysTenant, SysTenantVO.class);
            String targetUrl = redirectUrl + "/pages/about/concern?id=" + sysTenantVO.getId();
            //String base64 = QRCodeUtil.getQRCodeImage(targetUrl, "",null, sysTenant.getCompanyLogo());
            //获取二维码
            String base64 = QRCodeGeneratorUtil.creatQrImage(targetUrl, sysTenant.getCompanyLogo(),"");
            sysTenantVO.setQrCode(base64);
            result.setResult(sysTenantVO);
            result.setSuccess(true);
        }
        return result;
    }
    ...
    public Result<?> add(@RequestBody SysTenant sysTenant) {
        ...
        SysUserTenant sysUserTenant = relationService.getOne(new QueryWrapper<SysUserTenant>().
                lambda().eq(SysUserTenant::getUserId, sysUser.getId()));
        if (sysUserTenant != null) {
            return result.error500("该用户已注册企业信息，请勿重复提交!");
        }
        try {
            sysTenantService.saveTenant(sysTenant);
            SysUser sysUserUpdate = new SysUser();
            sysUserUpdate.setId(sysUser.getId());
            sysUserUpdate.setTenantId(sysTenant.getId().toString());
            if (StrUtil.isBlank(sysTenant.getTrade())) {
                sysTenant.setTrade("1");
            }
            //绑定租户信息
            sysUserService.updateById(sysUserUpdate);
            sendEmailMsg(sysTenant.getName());
            //添加
            result.success("添加成功！");
        } catch (Exception e) {
        ...
        result.setResult(sysTenant.getId());
        return result;
    }

    private void sendEmailMsg(String tenantName) {
        String[] emails = {"1099272020@qq.com", "1030298182@qq.com", "942496270@qq.com"};
        String title = "【聚印科技公司提醒】您有的新的企业注册";
        String content = "尊敬的聚印科技兄弟们，新的企业:【" + tenantName +"】注册了，请注意及时到PC端给用户审核通过！兄弟们辛苦了";
        for (int i = 0; i < emails.length; i++) {
            sysBaseAPI.sendEmailMsg(emails[i], title, content);
        }
    }
    ...
    public Result<SysTenant> edit(@RequestBody SysTenant tenant) {
        ...
        if (sysTenant.getAuditStatus().equals(2)) {
            tenant.setAuditStatus(0);//审核失败修改成审核中
        }
    ...
    *@Description: 租户审核
    *@Param: [tenant]
    *@Return: org.jeecg.common.api.vo.Result<org.jeecg.modules.system.entity.SysTenant>
    *@author: xiaopeng.wu
    *@DateTime: 13:52 2023/11/23
   **/
    @ApiOperation(value="租户权限-租户审核", notes="租户权限-租户审核")
    @RequiresPermissions("system:tenant:audit")
    @RequestMapping(value = "/audit", method ={RequestMethod.PUT})
    public Result<SysTenant> audit(@Validate @RequestBody TenantAuditDTO tenant) {
        Result<SysTenant> result = new Result();
        SysTenant sysTenant = sysTenantService.getById(tenant.getId());
        if(sysTenant==null) {
           return result.error500("未找到对应实体");
        }
        SysUser sysUser = sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getTenantId,tenant.getId()).eq(SysUser::getUserIdentity,3));
        //审核通过
        if (tenant.getAuditStatus().equals(1)) {
            SysUserRole userRole = new SysUserRole();
            userRole.setRoleId("2");//默认租户角色
            //去掉注册角色
            userRoleService.update(userRole, new UpdateWrapper<SysUserRole>().lambda().eq(SysUserRole::getUserId,sysUser.getId()));

            //审核通过通知
            TYSmsUtil.sendPassAudit(sysUser.getPhone());
        }
        if (tenant.getAuditStatus().equals(2)) {
            //审核未通过通知
            TYSmsUtil.sendNoPassAudit(sysUser.getPhone());
        }
        boolean ok = sysTenantService.updateById(BeanUtil.copyProperties(tenant, SysTenant.class));
        if(ok) {
            result.success("修改成功!");
        }
        return result;
    ...
    @ApiOperation(value = "删除租户信息-通过id")
